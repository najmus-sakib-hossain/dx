...
fn bench_serialize(c: &mut Criterion) {
    ...
    for size in &[50, 100, 500, 1000, 10000] {
...
fn print_size_comparison() {
    ...
    for size in &[50, 100, 500, 1000, 10000] {
        ...
        let rkyv_size: usize = rkyv_data.iter().map(rkyv::util::AlignedVec::len).sum();
        let dx_size: usize = dx_data.iter().map(rkyv::util::AlignedVec::len).sum();

        println!("  {size} items:");
        println!("    RKYV:       {rkyv_size:>8} bytes");
        println!("    DX-Machine: {dx_size:>8} bytes");
        println!(
            "    Difference: {:>8} bytes ({}%)",
