...
impl DxCompressed {
    ...
    pub fn decompress(&mut self) -> Result<&[u8]> {
        ...
        // SAFETY: We just set self.decompressed = Some(data) in the if block above,
        // so it is guaranteed to be Some at this point. Using expect() with a clear
        // message for the impossible case.
        #[allow(clippy::unwrap_used)] // Guaranteed Some after the if block above
...
#[allow(dead_code)]
#[cfg(all(feature = "compression", feature = "compression-zstd"))]
fn zstd_compress(input: &[u8], level: i32) -> Vec<u8> {
    zstd::encode_all(input, level).unwrap_or_else(|_| input.to_vec())
}

#[allow(dead_code)]
#[cfg(not(all(feature = "compression", feature = "compression-zstd")))]
...
pub fn compress_zstd(_data: &[u8]) -> Result<Vec<u8>> {
    Err(DxMachineError::InvalidData(
        "Zstd compression not available (enable 'compression-zstd' feature)".into(),
    ))
}
...
pub fn decompress_zstd(_data: &[u8]) -> Result<Vec<u8>> {
    Err(DxMachineError::InvalidData(
        "Zstd decompression not available (enable 'compression-zstd' feature)".into(),
    ))
}
