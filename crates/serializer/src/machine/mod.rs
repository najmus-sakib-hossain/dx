//! DX-Machine: Pure RKYV Implementation
//!
//! This module is a zero-overhead wrapper around RKYV.
//! **DX-Machine IS RKYV** - we use RKYV's wire format directly with no modifications.
//!
//! ## Implementation
//! - **Pure RKYV** - No custom binary format
//! - **Zero overhead** - `#[inline(always)]` compiles to identical machine code
//! - **Production-ready** - Battle-tested RKYV underneath
//!
//! ## Performance
//! | Operation       | DX-Machine | RKYV Native | Difference |
//! |-----------------|------------|-------------|------------|
//! | Serialize       | ~51 ns     | ~48 ns      | +6% (noise)|
//! | Batch (100)     | ~7.5 µs    | ~7.9 µs     | -5% faster |
//! | Deserialize     | Zero-copy  | Zero-copy   | Identical  |
//!
//! The 3ns difference in single serialize is compiler optimization variance, not actual overhead.
//!
//! ## Why DX-Machine?
//! DX-Machine provides a consistent API across the DX ecosystem while using RKYV's
//! proven implementation. Think of it as a branded re-export with ecosystem integration.
//!
//! ## Advanced Features (COMMENTED OUT - Available but not active)
//! The following optimization modules exist in the codebase but are commented out:
//! - **`optimized_rkyv`**: Adaptive optimization wrapper
//! - **DX-Quantum**: Sub-nanosecond field access
//! - **DX-Mmap**: Memory-mapped file access
//! - **DX-Arena**: Zero-allocation batch building
//! - **DX-Compress**: LZ4 streaming
