...
//! port=8080
//!
//! # Objects (inline with field count)
...
//!
//! The following features are implemented but commented out pending further evaluation:
//!
//! - **Prefix elimination**: `@prefix` removes repeated prefixes from table columns
//!   - Example: `logs:2(timestamp endpoint)@/api/ [10:23:45Z users, 10:24:12Z orders]`
//!   - Parses as: `10:23:45Z /api/users` and `10:24:12Z /api/orders`
//!
//! - **Compact syntax**: `@=` format for space-separated key-value pairs
//!   - Example: `config:3@=[host localhost port 8080 debug true]`
//!   - Equivalent to: `config:3[host=localhost port=8080 debug=true]`
...
impl<'a> DsrParser<'a> {
    ...
    fn parse_statement(&mut self, doc: &mut DxDocument) -> Result<(), ParseError> {
        ...
        if self.pos >= self.input.len() {
            ...
            doc.context.insert(name, DxLlmValue::Str(String::new()));
            return Ok(());
        }
        ...
        match ch {
            Some('[') => {
                // Object or array with count: name[key=value,...] or name[n]: items
                ...
                if !is_leaf && next == Some('(') {
                    ...
                    if self.peek_char() == Some('(') {
                        ...
                        if next_pos < self.input.len() && self.input[next_pos..].starts_with('=') {
                            ...
                            doc.context.insert(name, LlmParser::parse_value(&value_str));
                        }
                    } else if self.peek_char() == Some('[') {
    ...
    /// Parse a table value, converting underscores back to spaces for space-separated format
    /// Only converts underscores to spaces for patterns that look like human names (`First_Last`)
    fn parse_table_value(&self, s: &str, separator: char) -> DxLlmValue {
        let s = s.trim();
        ...
        // For space-separated format, convert underscores back to spaces
        // Only for patterns that look like human names: "First_Last" or "First_Middle_Last"
        // Requirements:
        // 1. Contains underscore
        // 2. Each part is in Title Case (first char uppercase, rest lowercase)
        // 3. Each part has at least 3 characters (to avoid "Aa_Bb" false positives)
        // 4. Each part is all letters (no numbers or special chars)
        // This prevents false positives like "AA_Aa", "Aa_Aa", "HTTP_Request", "MyVar_Name"
        if separator == ' ' && s.contains('_') {
            let parts: Vec<&str> = s.split('_').collect();
            if parts.len() >= 2 {
                let looks_like_name = parts.iter().all(|p| {
                    if p.len() < 3 {
                        return false;
                    }
                    let mut chars = p.chars();
                    // Safe: we already checked p.len() >= 3, so first char exists
                    let Some(first) = chars.next() else {
                        return false;
                    };
                    // First char must be uppercase, rest must be lowercase letters
                    first.is_uppercase() && chars.all(char::is_lowercase)
                });
                if looks_like_name {
                    return DxLlmValue::Str(s.replace('_', " "));
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_parse_table_with_underscore_names() {
        // Space-separated with underscore for text containing spaces
        let input = "employees:2(id name dept)[\n1 James_Smith Engineering\n2 Mary_Johnson Sales]";
        let doc = LlmParser::parse(input).unwrap();
        ...
        // Names with underscores that look like "First_Last" are converted to spaces
