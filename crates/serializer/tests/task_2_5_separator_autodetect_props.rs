...
fn simple_cell_value() -> impl Strategy<Value = String> {
    prop_oneof![
        ...
        "[a-zA-Z][a-zA-Z0-9_]{0,10}".prop_map(|s| s.clone()),
        // Numbers
...
proptest! {
    ...
    fn prop_separator_auto_detection_single_row(
        ...
        let input = format!(
            "{table_name}:1({schema_str})[{row_str}]"
        );
    ...
    fn prop_separator_auto_detection_empty(
        ...
        let schema_str = schema.join(" ");
        let input = format!("{table_name}:0({schema_str})[");
    ...
    fn prop_separator_auto_detection_multiple_tables(
        ...
        let schema1 = ["a".to_string(), "b".to_string()];
        ...
        let schema2 = ["x".to_string(), "y".to_string(), "z".to_string()];
