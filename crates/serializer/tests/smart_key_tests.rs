...
fn test_all_default_popular_keys() {
    ...
    for (full, short) in &mappings.compress {
        ...
        assert_eq!(result, *short, "Failed: {full} should compress to {short}");

        // Test expansion
        let expanded = mappings.expand_key(short);
        assert_eq!(expanded, *full, "Failed: {short} should expand to {full}");

        tested += 1;
    }

    println!("✅ All {tested} popular keys tested successfully!");
...
fn test_custom_keys_not_in_mappings() {
    ...
    for key in custom_keys {
        ...
        println!("  ✅ {key} → {compressed} (preserved)");
    }
}
