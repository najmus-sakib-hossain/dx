...
active=true";
    ...
    // Verify magic bytes
    assert_eq!(machine.data[0], b'D');
    assert_eq!(machine.data[1], b'X');
    assert_eq!(machine.data[2], b'M');
    assert_eq!(machine.data[3], b'F');
    assert_eq!(machine.data[4], 1); // version
...
"#;
    ...
    assert!(
        ...
        "Should have data: original={original_total}, roundtrip={roundtrip_total}"
    );
}
...
active=true";
    ...
    // Machine format should be compact
    // Header (5 bytes) + data
    assert!(
        machine.data.len() < 200,
...
fn test_empty_document_to_machine() {
    ...
    // Should have valid header even for empty document
    assert_eq!(machine.data[0], b'D');
    assert_eq!(machine.data[1], b'X');
    assert_eq!(machine.data[2], b'M');
    assert_eq!(machine.data[3], b'F');
