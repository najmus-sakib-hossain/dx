...
fn main() {
    ...
    for sr_file in &sr_files {
        ...
        if !sr_path.exists() {
            println!("⚠ Skipping {sr_file} (not found)");
            continue;
        }
        ...
        println!(
            ...
            machine.data[0], machine.data[1], machine.data[2], machine.data[3]
        );
    ...
    // Summary
    println!("=== Summary ===");
    println!("Total Dx Serializer size: {total_sr_size} bytes");
    println!("Total Machine size: {total_machine_size} bytes");
