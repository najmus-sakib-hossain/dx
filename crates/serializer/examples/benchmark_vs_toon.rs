...
fn main() {
    ...
    for (model, model_name) in &models {
        ...
        for (filename, label) in &examples {
            let toon_path = format!("essence/{filename}.toon");
            let dx_path = format!("essence/{filename}.sr");
            ...
            println!("{label:<18} {toon_tokens:>8} {dx_tokens:>8} {savings:>9.1}%");
    ...
    for (model, model_name) in &models {
        ...
        println!("{model_name:<12} {total_toon:>10} {total_dx:>10} {savings:>9.1}%");
    }
    ...
    println!("Average across all models: {avg_savings:.1}% savings\n");
