    ...
    fn render(&mut self, term: &Term) -> io::Result<()> {
        ...
        match self.state {
            State::Active => {
                ...
                let year_marker = if self.active_field == DateField::Year { "▸" } else { " " };
                let month_marker = if self.active_field == DateField::Month { "▸" } else { " " };
                let day_marker = if self.active_field == DateField::Day { "▸" } else { " " };
                ...
                term.write_line(&format!(
                    "{}  {} Year:  {}",
                    bar, year_marker, year_display
                ))?;
                lines += 1;

                term.write_line(&format!(
                    "{}  {} Month: {} ({})",
                    bar, month_marker, month_display, self.month_name()
                ))?;
                lines += 1;

                term.write_line(&format!(
                    "{}  {} Day:   {}",
                    bar, day_marker, day_display
                ))?;
                lines += 1;
