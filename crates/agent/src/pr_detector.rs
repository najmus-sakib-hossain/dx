...
impl PrDetector {
    ...
    pub async fn queue_new_integration(&self, name: &str, code: &str, language: &str) -> Result<()> {
        info!("Queuing new integration for PR: {}", name);
        
        // Save the integration with a .new marker
        let path = self.dx_path.join("integrations").join(format!("{}.{}", name, language));
    ...
    pub async fn create_pr(&self, diff: &LocalDiff) -> Result<()> {
        ...
        let pr_title = match diff.kind {
            DiffKind::NewIntegration => format!("feat(integration): add {}", diff.path.file_stem().unwrap().to_str().unwrap()),
            DiffKind::NewSkill => format!("feat(skill): add {}", diff.path.file_stem().unwrap().to_str().unwrap()),
            DiffKind::NewPlugin => format!("feat(plugin): add {}", diff.path.file_stem().unwrap().to_str().unwrap()),
            DiffKind::ConfigChange => format!("chore(config): update {}", diff.path.file_stem().unwrap().to_str().unwrap()),
            DiffKind::Other => format!("chore: update {}", diff.path.file_stem().unwrap().to_str().unwrap()),
        };
