...
impl AgentDaemon {
    ...
    pub async fn new(config: DaemonConfig) -> Result<Self> {
        ...
        let integrations = Arc::new(RwLock::new(
            IntegrationManager::new(&config.dx_path).await?
        ));
        
        // Initialize skill registry
        let skills = Arc::new(RwLock::new(
            SkillRegistry::new(&config.skills_path).await?
        ));
    ...
    async fn handle_message(&self, integration: &str, message: String) -> Result<()> {
        ...
        for skill_name in response.required_skills() {
            if let Some(skill) = skills.get(skill_name) {
                ...
                let context_str = response.context()
                    .iter()
                    .map(|(k, v)| format!("{}={}", k, v))
                    .collect::<Vec<_>>()
                    .join(" ");
    ...
    pub async fn create_integration(&self, name: &str, code: &str, language: &str) -> Result<()> {
        ...
        // Load the plugin
        let mut plugins = self.plugins.write().await;
        plugins.load_from_bytes(name, &wasm_bytes, &manifest).await?;
        ...
        if self.config.auto_pr {
            self.pr_detector.queue_new_integration(name, code, language).await?;
        }
