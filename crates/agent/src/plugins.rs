...
use crate::{Result, wasm_runtime::WasmRuntime};
...
impl PluginLoader {
    ...
    async fn load_from_path(&self, wasm_path: &Path, manifest_path: &Path) -> Result<Plugin> {
        ...
        self.wasm_runtime.load_module_from_file(&manifest.name, wasm_path).await?;
    ...
    pub async fn load_from_bytes(&mut self, name: &str, wasm_bytes: &[u8], manifest_dx: &str) -> Result<()> {
