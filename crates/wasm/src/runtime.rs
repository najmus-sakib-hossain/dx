...
use crate::{WasmError, Result, module::WasmModule};
...
impl WasmRuntime {
    ...
    pub async fn load(&self, name: &str, wasm_bytes: &[u8]) -> Result<()> {
        ...
        if &wasm_bytes[0..4] != b"\0asm" {
            return Err(WasmError::InvalidWasm("Invalid WASM magic number".to_string()));
        }
    ...
    pub async fn call(&self, module_name: &str, function: &str, args: &[WasmValue]) -> Result<WasmValue> {
        ...
        info!("Function {}::{} called with {} args", module_name, function, args.len());
        Ok(WasmValue::I32(0))
    }
    
    /// Call a function that returns a string
    pub async fn call_string(&self, module_name: &str, function: &str, args: &[WasmValue]) -> Result<String> {
